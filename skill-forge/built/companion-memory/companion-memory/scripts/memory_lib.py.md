# MD 兼容视图 — `memory_lib.py`

> **权威源文件**：同目录 `memory_lib.py`（本文件由 `skill-core` 自动生成，请勿手改；改源文件后重新 `batch build` 或运行 compat 同步。）

| 项 | 值 |
|----|-----|
| 类型 | `py` |
| 路径 | `scripts/memory_lib.py` |
...
```bash
python scripts/memory_lib.py
```

Skill Steps 引用路径：`scripts/memory_lib.py` 或 `refs/scripts/memory_lib.py`（编译后位于 `references/refs/`）。
